...
const PaymentConfirmation: React.FC<IPaymentData> = ({ reference, ...rest }) => {
    const { data, isLoading, error, status, refetch } = usePaymentVerification(reference);
    ...
    return (
        <div className="flex flex-grow justify-center items-center">
            <div className="w-full md:w-96 px-4 py-8 md:px-10 bg-gray-light shadow-lg rounded-md md:mx-20 mx-auto md:absolute md:bottom-6 md:left-0 fixed inset-x-0 bottom-0 flex flex-col justify-center items-center md:items-start">
                ...
                {
                    !isLoading && error && <p className="text-center text-primary">{error.message}</p>
                }
                {
                    !isLoading && data && (
...
<>
                                    ...
                                    <p className='text-center text-red-700 italic text-sm'>Check your spam folder if you provided your email address and did not receive an email. </p>
</>                                
)
                            }
                        </div>
