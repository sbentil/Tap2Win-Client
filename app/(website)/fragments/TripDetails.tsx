    ...
    <>
      <div className="bg-white w-[95%] mx-2 md:mx-0 md:w-full h-[65px] bg-transparent rounded-full flex justify-between items-center p-2">
        <div className="w-1/2 h-full flex gap-2 items-center justify-start">
          ...
          <div className="">
            <h1 className="text-sm md:text-lg text-primary font-medium font-CabinetGrotesk">
              {caption}
            </h1>
            <ul className="flex gap-1">
              {places?.map((place, index) => (
                <li
                  key={index}
                  className="text-xs text-[#777] font-BeVietnamPro"
                >
                  {place.place}
                  {index < places.length - 1 && <span> </span>}
                </li>
              ))}
              {/* {<p>Ticket Purchase closed. Awaiting Draw in 48 hours</p>} */}
            </ul>
          </div>
        </div>
...
const TripDetailsPage: React.FC<{ onBuyClose: () => void }> = ({
  onBuyClose,
}) => {
  const btnText = "CLOSED!";
