...
const Page = (props: Props) => {
  ...
  //   const [paymentData, setPaymentData] = React.useState<IPaymentData | null>(
  //     null
  //   );
  ...
  return (
    <div className="explore h-screen w-screen">
      ...
      <HeaderFragment buying={false} />

      <ImageCarousel />
      {/* {buying ? (
        <TokenPurchaseForm onClose={() => handleBuyClick(false)} />
      ) : payment && paymentData ? (
      ...
      <div className="w-full md:w-[750px] md:h-1/3 md:px-16 py-4 absolute bottom-4 md:bottom-6">
        ...
        <h1 className="p-4 text-white text-xl md:text-2xl leading-tight text-pretty font-semibold font-VietnamPro text-shadow-md">
          support the construction of 300 Solar Vaccine Shelters in deprived
          communities across Ghana
        </h1>
        <TripDetailsPage onBuyClose={() => {}} />
      </div>
    </div>
  );
};
